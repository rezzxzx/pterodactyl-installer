...
# ---------- UI ----------
# helper kompatibel dgn struktur official (welcome / output / error)
welcome(){
  echo -e "${C_B}┌──────────────────────────────────────────────┐${C_N}"
  echo -e "${C_B}│   Pterodactyl Installer v$SCRIPT_VERSION          │${C_N}"
  echo -e "${C_B}└──────────────────────────────────────────────┘${C_N}"
}
output(){ echo -e "${C_Y}*${C_N} $1"; }
error(){ echo -e "${C_R}! $1${C_N}"; }
show_help(){
  cat <<EOF
...
main(){
  ...
  # canary: strip suffix, script sudah set GITHUB_SOURCE=master di bootstrap
  action="${action%_canary}"
